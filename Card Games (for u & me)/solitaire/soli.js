...
function sm_inc(arr, i) { // Evan's string converter for game stats
...
function solitaireGame(oUniqueName,oWorkspace,oDeck,oWastePile,oFoundation,oTableau,oCardWord,oCardNames,oSuits,youWin,oDeal,oOptions,
	...
	dealDiv.onclick = function () {
		...
		S_ach[1] = 'y'; localStorage.setItem("S_localach", JSON.stringify(S_ach)); // ACHIEVEMENT: Deal cards
	...
	setTimeout(function () {
		if( theGame.ongamewon ) {
			if( theGame.ongamewon({type:'gamewon',target:theGame,currentTarget:theGame}) ) {
				...
				S_ach[4] = 'y'; localStorage.setItem("S_localach", JSON.stringify(S_ach)); // ACHIEVEMENT: Win a game
				...
				sm_inc(S_score, 1); localStorage.setItem("S_localscore", JSON.stringify(S_score)); // SCORE: Games won				
				
				theGame.startGame();
			}
		} else if( confirm(theGame.strings.youWin) ) { 
			var S_ach = ['n', 'n', 'n', 'n', 'n'];
			if (localStorage.getItem("S_localach") === null) {
				console.log("Local storage for S_ach does not exist");
				localStorage.setItem("S_localach", JSON.stringify(S_ach));
			...
			if (localStorage.getItem("S_localscore") === null) {
				console.log("Local storage for S_localscore does not exist");
				localStorage.setItem("S_localscore", JSON.stringify(S_score));
			} else {					console.log("Local storage for S_localscore exists");
				S_score = JSON.parse(localStorage.getItem("S_localscore"));
			}
			sm_inc(S_score, 1); localStorage.setItem("S_localscore", JSON.stringify(S_score)); // SCORE: Games won
...
solitaireGame.prototype.startGame = function (oInstantWin) {
	...
	S_ach[0] = 'y'; localStorage.setItem("S_localach", JSON.stringify(S_ach)); // ACHIEVEMENT: Play a game
	...
	sm_inc(S_score, 0); localStorage.setItem("S_localscore", JSON.stringify(S_score)); // SCORE: Games played	
...
solitaireGame.prototype.handleClicksOnCards = function (e) {
	...
	if( this.relatedObject.game.doingSpatNav ) {
		...
		S_ach[2] = 'y'; localStorage.setItem("S_localach", JSON.stringify(S_ach)); // ACHIEVEMENT: Flip dealt cards
		...
		S_ach[3] = 'y'; localStorage.setItem("S_localach", JSON.stringify(S_ach)); // ACHIEVEMENT: Move a card
